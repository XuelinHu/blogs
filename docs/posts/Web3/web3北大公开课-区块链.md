...
1. 广播传播 + 投票机制
   所有节点收到信息后互相广播；通过投票或选举方式决定采用哪条链、哪条交易。
2. 抗作恶设计
   作恶的成本必须远远高于收益；
...
---
## 7.2. ⛏️ 二、不同共识机制下的创币过程
### 7.2.1. 🔹（1）PoW：工作量证明（如比特币）
#### 7.2.1.1. ✅ 创币过程：
1. 节点收集交易，打包成一个新区块；
...
---
## 7.4. 📊 四、比特币创币交易（Coinbase Transaction）
...
### 10.2.1. ✅ 每个节点只维护一个**本地邻居列表（peer list）**
...
---
## 12.2. 🧪 二、第二代：**账户模型与状态树（以太坊）**
### 12.2.1. ⏳ 时间：2015 年
### 12.2.2. 🔧 代表项目：Ethereum
### 12.2.3. 🧱 数据结构升级：
| 模块                   | 描述                    |
|----------------------|-----------------------|
...
## 12.3. 🕸️ 三、第三代：**图结构（DAG）与分片尝试**

### 12.3.1. ⏳ 时间：2016 年后

### 12.3.2. 🔧 代表项目：IOTA、Nano、Avalanche、以太坊2.0
...
---
## 12.4. 🧠 四、现代结构：**ZK Rollup & 模块化架构（2020s）**
### 12.4.1. 🔧 代表项目：zkSync、StarkNet、Celestia、Near、EigenLayer
### 12.4.2. 🔬 数据结构创新：
| 技术                                   | 描述                                          |
|--------------------------------------|---------------------------------------------|
...
**
# 13. 区块链如何证明一个新的区块不存在
# 14. 区块链的全节点一直在线，那这个全节点是不是没有去中心化
...
# 15. 区块链发生了交易后，怎么通知每个用户，有用户离线了怎么办，是不是用类似版本号的方式，离线用户更改信息前，要先同步最新的版本
