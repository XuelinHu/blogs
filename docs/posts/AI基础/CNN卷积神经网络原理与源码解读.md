...
::: danger 注意：图像维度或预处理错了，模型仍可能“正常运行”
PyTorch 通常要求 `[B,C,H,W]`，OpenCV 读入的是 BGR，预训练模型还要求固定的缩放与归一化。把 `[B,H,W,C]`、RGB/BGR 或 0～255/0～1 混用时，有些 shape 仍能通过，结果却完全失真；生产推理必须复用训练时的预处理、固定类别映射，并对空图、坏图和尺寸上限做校验。
:::
...
::: danger 注意：推理忘记 eval() 会让同一图片得到漂移结果
上线加载权重后应显式调用 `model.eval()` 并在 `torch.inference_mode()` 下运行，同时用固定样本做启动自检。BatchNorm 统计量和类别表都属于模型制品的一部分；只复制参数、遗漏 buffer 或复用训练态进程，会造成难以复现的批次相关误判。
:::
