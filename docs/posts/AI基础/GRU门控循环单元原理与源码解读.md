...
::: danger 注意：设备风险预测不能把报警标签泄漏进输入
滑窗特征必须只使用预测时刻之前真正可获得的数据，按设备和时间划分训练/验证集，并在传感器断线、常量值和量程外输入时拒绝给出“安全”结论。GRU 的低风险概率不是保护指令，过流、过温和急停仍应由独立确定性系统处理。
:::
...
::: danger 注意：风险概率不能覆盖确定性保护阈值
温度、振动或电流已经超过设备硬限制时，应由规则和安全回路直接处置，不能因为 GRU 给出“正常”而继续运行。记录模型版本、窗口时间、原始传感器值和触发原因，才能在漏报后复盘；传感器断线也应判为数据无效，而不是补零后当作正常。
:::
