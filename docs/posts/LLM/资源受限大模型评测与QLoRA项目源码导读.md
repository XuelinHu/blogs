...
::: danger 注意：评测与训练脚本会执行模型代码并消耗大量资源
不要对未知仓库盲目启用 `trust_remote_code=True`，模型与数据先在隔离环境做来源和哈希校验。限制输入长度、磁盘配额、GPU 显存和并发任务，保存 checkpoint 前确认剩余空间；HumanEval 一类代码评测必须在无生产凭据、禁用敏感网络和文件访问的沙箱中运行。
:::
