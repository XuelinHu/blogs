...
::: danger 注意：Agent 输出不能直接放行产品或执行高权限动作
提示词、图片 OCR、检索内容和工具返回值都属于不可信输入。生产 Agent 必须使用最小权限工具、参数白名单、超时与调用次数上限、幂等任务号和全链路审计；CV 漏检要进入确定性规则或人工复核，不能让模型一句“合格”直接覆盖质量门禁。
:::
